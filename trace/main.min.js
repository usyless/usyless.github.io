var e=!0;const g=[],aa=[];
async function p(a,{v:b=[],buttons:c,o:d=[],onclose:f}={}){q();e=!1;return new Promise(k=>{const l=document.createElement("div"),r=document.createElement("div"),n=document.createElement("div");l.setAttribute("usy-overlay","");l.classList.add("fullscreen","blur");r.classList.add("popupOuter",...d);n.classList.add("popupInner",...d);const m=[];if("string"===typeof a){var t=document.createElement("div");t.classList.add("popupText");t.textContent=a;m.push(t)}else Array.isArray(a)?m.push(...a):m.push(a);
r.appendChild(n);if("string"===typeof c||null==c){t=document.createElement("div");t.classList.add("popupButtons");const h=document.createElement("button");h.classList.add("standardButton");h.textContent=c??"Ok";t.appendChild(h);h.addEventListener("click",()=>{const L=a.B?.();q();k(L??!0)})}else t=c;n.append(...m,t);l.addEventListener("click",h=>{h.target===h.currentTarget&&(q(),k(!1))});l.appendChild(r);b.push({target:window,type:"keydown",listener:h=>{"escape"===h.key.toLowerCase()&&l.click()}});
...
A.u=()=>{B.l()&&(A.j.src=B.src,A.j.width=3*B.clientWidth,A.j.height=3*B.clientHeight)};
...
c)}D.xHigh.nextElementSibling.setAttribute("y",(x/2).toString());D.xLow.nextElementSibling.setAttribute("y",(x/2).toString());D.yHigh.nextElementSibling.setAttribute("x",(w/2).toString());D.yLow.nextElementSibling.setAttribute("x",(w/2).toString())},J=[D.xHigh,D.xLow,D.yHigh,D.yLow],ja=()=>{ia.classList.remove("hidden");K.setAttributeNS(null,"width","0")},ka=()=>{document.getElementById("erasing").classList.add("hidden")},ia,K,M;ia=document.getElementById("erasing");K=ia.firstElementChild;
const B=document.getElementById("uploadedImage");B.g=a=>{const b=B.getBoundingClientRect(),c=a.clientX;a=a.clientY;return{x:c,y:a,xRel:c-b.left,yRel:a-b.top}};B.l=()=>B.src.startsWith("blob:");B.C=()=>{const a=N.get(B.src),b={};if(a){for(const c in D)b[c]=E(D[c]);a.lines=b}};B.A=()=>{const a=N.get(B.src).lines;for(const b in D)F(D[b],a[b]);ha();G()};const ma=a=>{p(a).then(()=>ma(a)).catch(()=>ma(a))};
...
.2*x&&F(D.yLow,x))):(ta(b.svg),da(),O({type:"getHistoryStatus"})))}};return a})(),O=a=>B.l()&&ua.postMessage({src:B.src,...a}),va=(a,b)=>{O({type:"eraseRegion",begin:a,end:b})},wa=()=>{O({type:"autoTrace",colourTolerance:document.getElementById("colourTolerance").value||u.colourTolerance})},P=(a,b,c=!1)=>{O({type:"snapLine",line:{name:a.id,position:E(a),direction:a.dataset.direction},direction:b,final:c})},ta=a=>{var b=document.getElementById("trace");const c=b.lastElementChild;b=b.firstElementChild;
c.setAttribute("d",a);c.setAttribute("stroke","#ff0000");c.setAttribute("stroke-width",ca);b.setAttribute("d",a);b.setAttribute("stroke-width",(1.5*ca).toString())};document.getElementById("autoPath").addEventListener("click",wa);document.getElementById("undo").addEventListener("click",()=>O({type:"undoTrace"}));document.getElementById("redo").addEventListener("click",()=>O({type:"redoTrace"}));document.getElementById("clearPath").addEventListener("click",()=>O({type:"clearTrace"}));
...
bottomPixel:E(D.yLow)},FR:{top:document.getElementById("FRHigher").value,topPixel:E(D.xHigh),bottom:document.getElementById("FRLower").value,bottomPixel:E(D.xLow)}};a(b)?p("Please fill in all required values to export (SPL and FR values)"):O(b)});document.getElementById("smoothTrace").addEventListener("click",()=>O({type:"smoothTrace"}));const N=new Map,Q=document.getElementById("fileInput");
var xa=document.getElementById("imageQueueInner"),ya=a=>{N.delete(a.src);O({type:"removeImage",src:a.src});URL.revokeObjectURL(a.src);a.remove()},za=()=>{xa.querySelector('img[class="selectedImage"]').scrollIntoView({inline:"center",behavior:"smooth"})},Ba=(a,b=!1)=>{const c=document.createElement("img"),d=document.getElementById("imageQueueInner");c.src=a;N.set(c.src,{initial:!0});c.addEventListener("dragstart",f=>f.preventDefault());c.addEventListener("click",f=>{f.preventDefault();f.stopPropagation();
...
r.getImageData(0,0,w,x),m=n.data,t=m.length;for(let h=0;h<t;h+=4)m[h]=255-m[h],m[h+1]=255-m[h+1],m[h+2]=255-m[h+2];r.putImageData(n,0,0)}document.getElementById("removeImage").click();l.toBlob(n=>{Ba(URL.createObjectURL(n),!0);q()})},{once:!0});p(d,{buttons:a})}else p("No valid image selected")});ba();Aa();Q.m=a=>{a=Array.from(a).filter(c=>c.type.startsWith("image/"));const b=a.length-1;0<a.length?(q(),a.forEach((c,d)=>{c=URL.createObjectURL(c);Ba(c,d===b)})):p("Invalid image/file(s) added!")};
Q.addEventListener("change",a=>{Q.m(a.target.files)});B.addEventListener("dragstart",a=>a.preventDefault());document.getElementById("fileInputButton").addEventListener("click",()=>Q.click());document.addEventListener("paste",a=>{a.preventDefault();const b=new DataTransfer;for(const c of a.clipboardData.items)"file"===c.kind&&b.items.add(c.getAsFile());0<b.files.length&&Q.m(b.files)});R("dragover",document.body,a=>{a.preventDefault();a.dataTransfer.dropEffect="copy";document.body.classList.add("lowOpacity")});
...
B.addEventListener("pointermove",a=>{if("erase"!==y){a.preventDefault();const b=B.parentElement,c=b.getBoundingClientRect();a=B.g(a);A.style.left=`${Math.min(a.x-c.left,b.clientWidth-A.clientWidth)}px`;A.style.top=`${Math.min(a.y-c.top,b.clientHeight-A.clientHeight)}px`;A.j.style.left=`${-1*(3*a.xRel-A.clientWidth/2)}px`;A.j.style.top=`${-1*(3*a.yRel-A.clientHeight/2)}px`;O({type:"getPixelColour",x:a.xRel*v,y:a.yRel*v});A.classList.remove("hidden")}});
R(["pointerleave","pointerout","pointercancel"],B,()=>A.classList.add("hidden"));let S=null,Ca=B.g;C.addEventListener("pointerdown",a=>{const b=Ca(a);a={x:.02*w,y:.02*x};for(var c of J)c.offset=b[`${c.dataset.direction}Rel`]*v-E(c);c=J.reduce((d,f)=>Math.abs(f.offset)<Math.abs(d.offset)?f:d,J[0]);Math.abs(c.offset)<a[c.dataset.direction]&&(S=c)});C.addEventListener("pointermove",a=>{S&&F(S,Ca(a)[`${S.dataset.direction}Rel`]*v-S.offset)});
R(["pointerup","pointerleave","pointercancel"],C,a=>{a.preventDefault();S=null});let T,Da,Ea=document.getElementById("snapToLines").checked;document.getElementById("snapToLines").addEventListener("change",()=>Ea=document.getElementById("snapToLines").checked);
document.getElementById("buttonSection").addEventListener("pointerdown",a=>{const b=a.target,c=b.parentNode;!T&&c.classList.contains("moveButtons")&&(a.preventDefault(),Da=D[c.dataset["for"]],Ea?P(D[c.dataset["for"]],parseInt(b.dataset.direction,10)):T=setInterval(()=>{F(Da,E(Da)+parseInt(b.dataset.direction,10)*v)},10))});R(["pointerup","pointerleave","pointerout","pointercancel"],document.getElementById("buttonSection"),a=>{a.preventDefault();clearInterval(T);T=null});
window.addEventListener("resize",()=>{v=w/B.clientWidth;fa();A.u()});const Fa={path:(a,b)=>{O({type:"trace",x:a,y:b,colourTolerance:document.getElementById("colourTolerance").value||u.colourTolerance})},point:(a,b)=>O({type:"addPoint",x:a,y:b})};B.addEventListener("pointerup",a=>{null!=y&&(a=B.g(a),Fa[y]?.(a.xRel*v,a.yRel*v))});let U=!1;
B.addEventListener("pointerdown",a=>{"erase"===y&&(a.preventDefault(),U=!0,a=B.g(a).xRel*v,ja(),M=Math.max(Math.min(+a,w),0),K.setAttributeNS(null,"x",a),document.addEventListener("pointerup",Ga,{once:!0}))});B.addEventListener("pointermove",a=>{U&&"erase"===y&&(a.preventDefault(),a=B.g(a).xRel*v,a=+a,a<M?(K.setAttributeNS(null,"width",`${M-a}px`),K.setAttributeNS(null,"x",a)):(K.setAttributeNS(null,"width",`${a-M}px`),K.setAttributeNS(null,"x",M.toString())))});
const Ga=a=>{U&&"erase"===y&&(a.preventDefault(),U=!1,a=B.g(a).xRel*v,a=Math.max(Math.min(+a,w),0),va(...(M<a?[M,a]:[a,M])),K.setAttributeNS(null,"width","0"))};
B.addEventListener("load",()=>{document.getElementById("defaultMainText").classList.add("hidden");for(var a of document.querySelectorAll("[data-disabled]"))a.disabled=!1;na();w=B.naturalWidth;x=B.naturalHeight;ca=.003*Math.max(w,x);v=w/B.clientWidth;ka();for(var b of document.querySelectorAll("svg"))b.setAttribute("width",w),b.setAttribute("height",x),b.setAttribute("viewBox",`0 0 ${w} ${x}`);ta("");A.u();a=N.get(B.src);if(a.initial){document.querySelector('.waiting-overlay[data-for="trace"]').classList.add("enabled");
console.time("Initialise image");b=w;var c=x,d=document.createElement("canvas");const f=d.getContext("2d");d.width=b;d.height=c;f.drawImage(B,0,0);d=f.getImageData(0,0,b,c);ua.postMessage({src:B.src,type:"setData",data:d.data,width:b,height:c},[d.data.buffer]);F(D.xHigh,w);F(D.xLow,0);F(D.yHigh,0);F(D.yLow,x);ha();G();P(D.xHigh,-1);P(D.xLow,1);P(D.yHigh,1);P(D.yLow,-1,!0);wa();a.initial=!1}else O({type:"setCurrent"}),B.A(),O({type:"getCurrentPath"});fa()});
...
function La(){function a(){Z>=X.length-1?n.textContent="End Tutorial":0>=Z?(n.textContent="Begin Tutorial",m.disabled=!0,m.textContent=""):(n.textContent="Next",m.disabled=!1,m.textContent="Previous");b()}function b(){const t=X[Z]??X[0];k.textContent=t.i;r.innerHTML=t.body}Z=0;const c=document.createElement("div"),d=document.createElement("div");d.classList.add("popupButtons");c.classList.add("popupText");const f=document.createElement("div"),k=document.createElement("h1"),l=document.createElement("button");
f.classList.add("headContainer");l.classList.add("standardButton");l.textContent="\ud800\udf22";l.addEventListener("click",q);f.append(k,l);const r=document.createElement("p");c.append(r);const n=document.createElement("button"),m=document.createElement("button");n.classList.add("standardButton");m.classList.add("standardButton");n.addEventListener("click",()=>{Z>=X.length-1&&l.click();++Z;a()});m.addEventListener("click",()=>{--Z;a()});a();b();d.append(m,n);p([f,c,d],{buttons:d,o:["fixedSize"],onclose:Ma})}
...
for(var n=0;16>n;++n){var m=document.createElement("div");m.style.setProperty("--i",n.toString());r.appendChild(m)}r.classList.add("line");d.append(f,k,l,r);let t,h;b.addEventListener("pointermove",Y=>{Y.target===Y.currentTarget&&(clearTimeout(t),c=c??b.getBoundingClientRect(),d.style.setProperty("--dy",`${(Y.clientX-c.left-c.width/2)/5}deg`),d.style.setProperty("--dx",`${(c.height/2-(Y.clientY-c.top))/5}deg`),null==h&&(d.classList.add("quick"),h=setTimeout(()=>{d.classList.add("no-transition");d.classList.remove("quick")},
200)))});b.addEventListener("pointerleave",()=>{clearTimeout(h);h=null;t=setTimeout(()=>{d.classList.remove("quick");d.classList.remove("no-transition");d.style.removeProperty("--dx");d.style.removeProperty("--dy")},100)});f=document.createElement("div");f.classList.add("popupInner");k=document.createElement("h2");k.textContent="UsyTrace";l=document.createElement("h3");l.textContent="made with \u2764\ufe0f by usy";r=document.createElement("h6");r.textContent="i made the 3d icon while procrastinating studying for exams, try hovering over it or dragging it";
n=document.createElement("div");n.textContent="Find me at:";m=document.createElement("div");m.textContent="Discord: @usy_";const L=document.createElement("div");var H=document.createElement("a");L.textContent="Email: ";H.href="mailto:usy@usyless.uk";H.textContent="usy@usyless.uk";L.appendChild(H);H=document.createElement("div");const la=document.createElement("a");H.textContent="GitHub: ";la.href="https://github.com/usyless/UsyTrace/issues";la.textContent="usyless";H.appendChild(la);f.append(k,l,
n,m,L,H,r);a.append(b,f);p(a,{v:[{target:window,type:"resize",listener:()=>{c=b.getBoundingClientRect()}}]})});"serviceWorker"in navigator&&!["localhost","127.0.0.1"].includes(location.hostname)&&(navigator.serviceWorker.addEventListener("controllerchange",()=>{window.location.reload()}),navigator.serviceWorker.register("./service-worker.js").then(a=>{a.addEventListener("updatefound",()=>{const b=a.installing;b.addEventListener("statechange",()=>{if("installed"===b.state&&navigator.serviceWorker.controller){const c=document.getElementById("updateAvailable");c.addEventListener("click",()=>{b.postMessage({action:"skipWaiting"})});
c.classList.remove("hidden")}})})}));const Na={default:"\u27a1\ufe0f\ud83d\udcbb Follow system",dark:"\ud83c\udf11 Default dark",light:"\u2600\ufe0f Default light","solarized-dark":"\ud83d\udfe6 Solarized dark","solarized-light":"\ud83c\udf68 Solarized light"};let Oa;
const Pa=a=>{for(const b of Array.from(document.documentElement.classList))b.startsWith("theme-")&&document.documentElement.classList.remove(b);clearTimeout(Oa);document.documentElement.classList.add("transition-theme");Oa=setTimeout(()=>{document.documentElement.classList.remove("transition-theme")},200);"default"===a?window.localStorage.removeItem("theme"):window.localStorage.setItem("theme",a);window.dispatchEvent(new CustomEvent("themeChange"))};
document.getElementById("themeSwitch").addEventListener("click",()=>{const a=document.createElement("div");var b=document.createElement("h3");a.classList.add("themePopupInner");b.textContent="Theme";a.appendChild(b);for(const c in Na)b=document.createElement("button"),b.classList.add("standardButton"),b.textContent=Na[c],b.dataset.theme=c,a.appendChild(b);a.addEventListener("click",c=>{if(c=c.target.closest("[data-theme]")?.dataset?.theme)Pa(c),q()});p(a,{buttons:"Exit",o:["themePopup"]})});
