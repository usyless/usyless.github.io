...
arrowright:a=>document.querySelector(`[data-for="x${a.shiftKey?"Low":"High"}"] > [data-direction="1"]`).dispatchEvent(X)};document.addEventListener("keydown",a=>{if(e){const b=Ha[a.key.toLowerCase()];!a.target.closest("input")&&b&&(a.preventDefault(),b(a))}});document.addEventListener("keyup",a=>{if(e){const b=Ia[a.key.toLowerCase()];!a.target.closest("input")&&b&&(a.preventDefault(),b(a))}});function S(a,b,c){for(const d of Array.isArray(a)?a:[a])b.addEventListener(d,c)}
