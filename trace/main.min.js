...
async function h(a,{A:b=[],buttons:c,v:d=[],onclose:e,F:m,C:n}={}){n||r();f=!1;return new Promise(k=>{var g=document.createElement("div"),l=document.createElement("div"),u=document.createElement("div");g.setAttribute("usy-overlay","");g.classList.add("fullscreen","blur");l.classList.add("popupOuter",...d);u.classList.add("popupInner",...d);var p=[];if(typeof a==="string"){var B=document.createElement("div");B.classList.add("popupText");B.textContent=a;p.push(B)}else Array.isArray(a)?p.push(...a):
p.push(a);l.appendChild(u);if(typeof c==="string"||c==null){B=document.createElement("div");B.classList.add("popupButtons");let q=document.createElement("button");q.classList.add("standardButton");q.textContent=c??"Ok";B.appendChild(q);q.addEventListener("click",()=>{var P=a.D?.();n?r(null,g):r();k(P??!0)});ia=q}else B=c;u.append(...p,B);g.addEventListener("click",q=>{q.target===q.currentTarget&&(n?r(null,g):r(),k(!1))});g.appendChild(l);b.push({target:window,type:"keydown",listener:q=>{q.key.toLowerCase()===
"escape"&&g.click()}});for(let q of b)fa.push(q),q.target.addEventListener(q.type,q.listener);e!=null&&ha.push(e);m?.(g);document.body.appendChild(g)})}function r(a,b=null){if(b)b.remove();else{f=!0;for(let c of fa)c.target?.removeEventListener?.(c.type,c.listener);for(let c of ha)c();ha.length=0;fa.length=0;document.querySelectorAll("[usy-overlay]").forEach(c=>c.remove())}};const t={FRHigher:"",FRLower:"",colourTolerance:67,line_move_speed:100,PPO:48,delimitation:"tab",lowFRExport:20,highFRExport:2E4,SPLHigher:"",SPLLower:""};document.getElementById("restoreDefault").addEventListener("click",()=>{ja();h("Restored settings to default")});function ja(){for(let a in t)document.getElementById(a).value=t[a]}const v=document.createElement("canvas"),w=v.getContext("2d");let x,y,z,ka,A=null,C=null;const D=document.getElementById("glass");D.l=D.querySelector("img");
D.I=a=>{D.style.borderColor=`rgb(${a})`};D.B=()=>{E.m()&&(D.l.src=E.src,D.l.width=E.clientWidth*3,D.l.height=E.clientHeight*3)};
var la=()=>{document.querySelector('.waiting-overlay[data-for="trace"]').classList.remove("enabled")},F=document.getElementById("lines"),G={xHigh:document.getElementById("xHigh"),xLow:document.getElementById("xLow"),yHigh:document.getElementById("yHigh"),yLow:document.getElementById("yLow")},ma=(a,b)=>{var c=a.dataset.direction;a.nextElementSibling.setAttribute(c,b);a.setAttribute(`${c}1`,b);a.setAttribute(`${c}2`,b)},na=()=>{for(let a of F.querySelectorAll("line"))a.setAttribute("stroke-width",x);
for(let a of F.querySelectorAll("text"))a.setAttribute("font-size",`${1.3*x}em`)},H=a=>parseFloat(a.getAttribute(`${a.dataset.direction}1`)),I=(a,b)=>{var c=H(G[a.dataset.other]),d=a.dataset.direction==="x"?y:z;a===G.xHigh||a===G.yLow?ma(a,Math.max(c+1,Math.min(d-1,b))):ma(a,Math.max(1,Math.min(c-1,b)))},J=()=>F.classList.remove("hidden"),K=()=>F.classList.add("hidden"),oa=()=>{for(let a of L){let [b,c]=a.dataset.direction==="x"?["y",z]:["x",y];a.setAttribute(`${b}1`,"0");a.setAttribute(`${b}2`,c)}G.xHigh.nextElementSibling.setAttribute("y",
(z/2).toString());G.xLow.nextElementSibling.setAttribute("y",(z/2).toString());G.yHigh.nextElementSibling.setAttribute("x",(y/2).toString());G.yLow.nextElementSibling.setAttribute("x",(y/2).toString())},L=[G.xHigh,G.xLow,G.yHigh,G.yLow],qa=()=>{pa.classList.remove("hidden");M.setAttributeNS(null,"width","0")},ra=()=>{document.getElementById("erasing").classList.add("hidden")},pa,M,N;pa=document.getElementById("erasing");M=pa.firstElementChild;const E=document.getElementById("uploadedImage");
E.i=a=>{var b=E.getBoundingClientRect(),c=a.clientX;a=a.clientY;return{x:c,y:a,xRel:c-b.left,yRel:a-b.top}};E.m=()=>E.src.startsWith("blob:");E.H=()=>{var a=O.get(E.src),b={};if(a){for(let c in G)b[c]=H(G[c]);a.lines=b}};E.G=()=>{var a=O.get(E.src).lines;for(let b in G)I(G[b],a[b]);oa();J()};
const Q=(()=>{var a=document.getElementById("SPLHigher"),b=document.getElementById("SPLLower"),c=document.getElementById("FRHigher"),d=document.getElementById("FRLower"),e=document.getElementById("snapToLines"),m=document.getElementById("line_move_speed"),n=document.getElementById("colourTolerance"),k=document.getElementById("PPO"),g=document.getElementById("delimitation"),l=document.getElementById("lowFRExport"),u=document.getElementById("highFRExport");return{SPLHigher:()=>a.value,SPLLower:()=>
...
var ta=()=>{for(let a of document.querySelectorAll("#sidebar [data-default]"))a.textContent=a.dataset["default"];A=null;C?.()},ua=a=>{document.getElementById("undo").disabled=!a.undo;document.getElementById("redo").disabled=!a.redo};
const va=["selectPath","selectPoint","eraseRegion"],wa={path:K,point:K,erase:()=>{K();qa()}},xa={path:J,point:J,erase:()=>{ra();J()}},ya=a=>{a=a.target;var b=a.dataset.mode,c=JSON.parse(JSON.stringify(A));ta();c===b?(C?.(),A=null):(a.textContent=a.dataset.active,C?.(),C=xa[b],A=b,wa[b]())};for(let a of va)document.getElementById(a).addEventListener("click",ya);
var Aa=(()=>{var a=new Worker("./usytrace.js",{type:"module"});a.onmessage=b=>{b=b.data;var c=b.type;switch(c){case "exportTrace":c=document.createElement("div");let d=document.createElement("div"),e=document.createElement("input");d.textContent="Export file name";e.placeholder="trace";e.type="text";e.classList.add("sidebarSection");c.append(d,e);c.classList.add("exportBox");c.D=()=>e.value;h(c,{buttons:"Save Trace",A:[{target:document,type:"keydown",listener:m=>{m.key.toLowerCase()==="enter"&&ia.click()}}]}).then(m=>
{if(m!==!1){let n=document.createElement("a"),k=URL.createObjectURL(new Blob([b["export"]],{type:"text/plain;charset=utf-8"}));n.href=k;n.classList.add("hidden");document.body.appendChild(n);!m?.endsWith(".txt")&&m?.length>0&&(m+=".txt");n.download=m||"trace.txt";n.click();setTimeout(()=>{URL.revokeObjectURL(k);document.body.removeChild(n)},5E3)}});setTimeout(()=>{e.focus()},50);break;case "error":la();sa(b.message);break;case "getHistoryStatus":ua(b);break;case "setData":console.timeEnd("Initialise image");
...
line:{name:a.id,position:H(a),direction:a.dataset.direction},direction:b,final:c})},za=a=>{var b=document.getElementById("trace"),c=b.lastElementChild;b=b.firstElementChild;c.setAttribute("d",a);c.setAttribute("stroke","#ff0000");c.setAttribute("stroke-width",ka);b.setAttribute("d",a);b.setAttribute("stroke-width",(ka*1.5).toString())};document.getElementById("autoPath").addEventListener("click",Ca);document.getElementById("undo").addEventListener("click",()=>R({type:"undoTrace"}));
...
document.getElementById("editImage").addEventListener("click",()=>{if(E.m()){let d=document.createElement("div");var a=document.createElement("h3"),b=document.createElement("div"),c=document.createElement("div");let e=document.createElement("img");d.id="editContainer";d.append(a,b,c);c.append(e);c.classList.add("cropWrapper");e.draggable=!1;a.textContent="Edit Image";e.src=E.src;let m=new Set,n=!("filter"in CanvasRenderingContext2D.prototype);a=n?{Invert:"invert(1)"}:{Invert:{g:"invert",default:1,
unit:""},Brightness:{g:"brightness",default:120,unit:"%",description:"Enter value for brightness in %",validate:k=>Math.max(0,k)},Contrast:{g:"contrast",default:120,unit:"%",description:"Enter value for contrast in %",validate:k=>Math.max(0,k)},Saturate:{g:"saturate",default:120,unit:"%",description:"Enter value for saturation in %",validate:k=>Math.max(0,k)},Hue:{g:"hue-rotate",default:5,unit:"deg",description:"Enter value for hue rotation in degrees",validate:k=>Math.abs(k)%360}};for(let k in a){c=
document.createElement("button");c.textContent=k;c.classList.add("standardButton");b.appendChild(c);let g=a[k];c.addEventListener("click",()=>{if(m.has(k))m.delete(k),e.style.filter=e.style.filter.replaceAll(new RegExp(`${g.g}\\(.*\\)`,"g"),"");else if(g.validate){let l=document.createElement("input");l.placeholder=g.default;l.type="number";let u=[document.createTextNode(g.description),l];u.D=()=>l.value||g.default;h(u,{C:!0}).then(p=>{p!==!1&&Number.isFinite(Number(p))?(p=g.validate(p),m.add(k),
...
w.drawImage(e,0,0);if(n){console.log("Using fallback invert mode");let g=w.getImageData(0,0,y,z),l=g.data,u=l.length;for(let p=0;p<u;p+=4)l[p]=255-l[p],l[p+1]=255-l[p+1],l[p+2]=255-l[p+2];w.putImageData(g,0,0)}var k=U();v.toBlob(g=>{Ha(URL.createObjectURL(g),!0);k?.o();r();v.width=0;v.height=0})},{once:!0});h(d,{buttons:b})}else h("No valid image selected")});ja();Ga();
...
document.getElementById("buttonSection").addEventListener("pointerdown",a=>{var b=a.target,c=b.parentNode;if(!X&&c.classList.contains("moveButtons"))if(a.preventDefault(),a=c.dataset["for"])if(Ja=G[a],Ka)S(G[c.dataset["for"]],parseInt(b.dataset.direction,10));else{let d=parseInt(b.dataset.direction,10);X=setInterval(()=>{I(Ja,H(Ja)+d*x)},100/Q.line_move_speed()*10)}else{let d=parseInt(b.dataset.direction,10);X=setInterval(()=>{R({type:"offsetTrace",direction:d,magnitude:x})},100/Q.line_move_speed()*
...
console.time("Initialise image");b=y;var c=z;v.width=b;v.height=c;w.drawImage(E,0,0);let d=w.getImageData(0,0,b,c);Aa.postMessage({src:E.src,type:"setData",data:d.data,width:b,height:c},[d.data.buffer]);v.width=0;v.height=0;I(G.xHigh,y);I(G.xLow,0);I(G.yHigh,0);I(G.yLow,z);oa();J();S(G.xHigh,-1);S(G.xLow,1);S(G.yHigh,1);S(G.yLow,-1,!0);Ca();a.initial=!1}else R({type:"setCurrent"}),E.G(),R({type:"getCurrentPath"});na()});E.addEventListener("error",()=>{if(E.m()){for(let a of U())Ea(a);h("Error loading this image, it may be malformed")}});
...
function tb(){function a(){Z>=rb.length-1?g.textContent="End Tutorial":Z<=0?(g.textContent="Begin Tutorial",l.disabled=!0,l.textContent=""):(g.textContent="Next",l.disabled=!1,l.textContent="Previous");b()}function b(){var u=rb[Z]??rb[0];m.textContent=u.j;k.innerHTML=u.body}Z=0;var c=document.createElement("div"),d=document.createElement("div");d.classList.add("popupButtons");c.classList.add("popupText");var e=document.createElement("div"),m=document.createElement("h1"),n=document.createElement("button");
e.classList.add("headContainer");n.classList.add("standardButton");n.textContent="\ud800\udf22";n.addEventListener("click",r);e.append(m,n);var k=document.createElement("p");c.append(k);var g=document.createElement("button"),l=document.createElement("button");g.classList.add("standardButton");l.classList.add("standardButton");g.addEventListener("click",()=>{Z>=rb.length-1&&n.click();++Z;a()});l.addEventListener("click",()=>{--Z;a()});a();b();d.append(l,g);h([e,c,d],{buttons:d,v:["fixedSize"],onclose:ub})}
function ub(){window.localStorage.setItem("USER_TUTORIAL_VERSION",(3).toString())}document.getElementById("tutorial").addEventListener("click",tb);document.getElementById("about").addEventListener("click",()=>{var a=document.createElement("div");a.classList.add("popupInner","about");var b=document.createElement("div"),c=null,d=document.createElement("div");d.classList.add("logo");b.appendChild(d);b.classList.add("logoWrapper");var e=document.createElement("div"),m=document.createElement("div"),n=document.createElement("div");e.classList.add("wall");m.classList.add("wall");n.classList.add("wall");var k=document.createElement("div");for(var g=
0;g<16;++g){var l=document.createElement("div");l.style.setProperty("--i",g.toString());k.appendChild(l)}k.classList.add("line");d.append(e,m,n,k);var u,p;b.addEventListener("pointermove",da=>{da.target===da.currentTarget&&(clearTimeout(u),c=c??b.getBoundingClientRect(),d.style.setProperty("--dy",`${(da.clientX-c.left-c.width/2)/5}deg`),d.style.setProperty("--dx",`${(c.height/2-(da.clientY-c.top))/5}deg`),p==null&&(d.classList.add("quick"),p=setTimeout(()=>{d.classList.add("no-transition");d.classList.remove("quick")},
