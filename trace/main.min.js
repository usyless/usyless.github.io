...
image$$module$main.isValid()&&worker$$module$main.worker.postMessage({src:image$$module$main.src,...a}),setCurrent:()=>worker$$module$main.postMessage({type:"setCurrent"}),removeImage:a=>worker$$module$main.postMessage({type:"removeImage",src:a}),addImage:(a,b)=>{var c=document.createElement("canvas");const d=c.getContext("2d");c.width=a;c.height=b;d.drawImage(image$$module$main,0,0);c=d.getImageData(0,0,a,b);worker$$module$main.worker.postMessage({src:image$$module$main.src,type:"setData",data:c.data,
width:a,height:b},[c.data.buffer])},clearTrace:()=>worker$$module$main.postMessage({type:"clearTrace"}),undoTrace:()=>worker$$module$main.postMessage({type:"undoTrace"}),redoTrace:()=>worker$$module$main.postMessage({type:"redoTrace"}),eraseRegion:(a,b)=>worker$$module$main.postMessage({type:"eraseRegion",begin:a,end:b}),smoothTrace:()=>worker$$module$main.postMessage({type:"smoothTrace"}),exportTrace:()=>{const a=c=>Object.values(c).some(d=>d&&"object"===typeof d?a(d):null===d||""===d),b={type:"exportTrace",
PPO:preferences$$module$main.PPO(),delim:preferences$$module$main.delimitation(),lowFR:preferences$$module$main.lowFRExport(),highFR:preferences$$module$main.highFRExport(),SPL:{top:preferences$$module$main.SPLHigher(),topPixel:lines$$module$main.getPosition(lines$$module$main.lines.yHigh),bottom:preferences$$module$main.SPLLower(),bottomPixel:lines$$module$main.getPosition(lines$$module$main.lines.yLow)},FR:{top:preferences$$module$main.FRHigher(),topPixel:lines$$module$main.getPosition(lines$$module$main.lines.xHigh),
bottom:preferences$$module$main.FRLower(),bottomPixel:lines$$module$main.getPosition(lines$$module$main.lines.xLow)}};a(b)?createPopup$$module$popups("Please fill in all required values to export (SPL and FR values)"):worker$$module$main.postMessage(b)},addPoint:(a,b)=>worker$$module$main.postMessage({type:"addPoint",x:a,y:b}),autoTrace:()=>{worker$$module$main.postMessage({type:"autoTrace",colourTolerance:preferences$$module$main.colourTolerance()})},trace:(a,b)=>{worker$$module$main.postMessage({type:"trace",
x:a,y:b,colourTolerance:preferences$$module$main.colourTolerance()})},snapLine:(a,b)=>{worker$$module$main.postMessage({type:"snapLine",line:{name:a.id,position:lines$$module$main.getPosition(a),direction:a.dataset.direction},direction:b})},getPixelColour:(a,b)=>worker$$module$main.postMessage({type:"getPixelColour",x:a,y:b}),getCurrentPath:()=>worker$$module$main.postMessage({type:"getCurrentPath"})},graphs$$module$main={updateSize:()=>{for(const a of document.querySelectorAll("svg"))a.setAttribute("width",
width$$module$main),a.setAttribute("height",height$$module$main),a.setAttribute("viewBox",`0 0 ${width$$module$main} ${height$$module$main}`)},setTracePath:a=>{var b=document.getElementById("trace");const c=b.lastElementChild;b=b.firstElementChild;c.setAttribute("d",a);c.setAttribute("stroke","#ff0000");c.setAttribute("stroke-width",lineWidth$$module$main);b.setAttribute("d",a);b.setAttribute("stroke-width",1.5*lineWidth$$module$main)},clearTracePath:()=>{graphs$$module$main.setTracePath("")}};
document.getElementById("autoPath").addEventListener("click",worker$$module$main.autoTrace);document.getElementById("undo").addEventListener("click",worker$$module$main.undoTrace);document.getElementById("redo").addEventListener("click",worker$$module$main.redoTrace);document.getElementById("clearPath").addEventListener("click",worker$$module$main.clearTrace);document.getElementById("export").addEventListener("click",worker$$module$main.exportTrace);
document.getElementById("smoothTrace").addEventListener("click",worker$$module$main.smoothTrace);
...
document.getElementById("buttonSection").addEventListener("pointerdown",a=>{const b=a.target,c=b.parentNode;!holdInterval&&c.classList.contains("moveButtons")&&(a.preventDefault(),line$jscomp$14=lines$$module$main.lines[c.dataset.for],snap?worker$$module$main.snapLine(lines$$module$main.lines[c.dataset.for],parseInt(b.dataset.direction,10)):holdInterval=setInterval(()=>{lines$$module$main.setPosition(line$jscomp$14,lines$$module$main.getPosition(line$jscomp$14)+parseInt(b.dataset.direction,10)*sizeRatio$$module$main)},
...
for(var h=0;16>h;++h){var f=document.createElement("div");f.style.setProperty("--i",h.toString());m.appendChild(f)}m.classList.add("line");d.append(e,n,g,m);let l,k;b.addEventListener("pointermove",q=>{q.target===q.currentTarget&&(clearTimeout(l),c=c??b.getBoundingClientRect(),d.style.setProperty("--dy",`${(q.clientX-c.left-c.width/2)/5}deg`),d.style.setProperty("--dx",`${(c.height/2-(q.clientY-c.top))/5}deg`),null==k&&(d.classList.add("quick"),k=setTimeout(()=>{d.classList.add("no-transition");d.classList.remove("quick")},
